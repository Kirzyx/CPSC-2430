...
int CovidDb::hash(std::string country, int size) {
    int sum = 0;
...
bool CovidDb::add(DataEntry* entry) {
    //find hash
    int index = hash(entry->getCountry(), tableSize);
...
void CovidDb::remove(std::string country) {
    // Find hash
    int index = hash(country, tableSize);
...
void CovidDb::calculate(std::string date, std::string country, int cases, int deaths) {
    ...
    std::vector<DataEntry*>& entries = dataTable[hash(country, tableSize)];
...
int CovidDb::findNextPrime(int num) {
    while (true) {
        bool isPrime = true;
        if (num <= 1)
            isPrime = false;
...
void CovidDb::rehash(int newSize) {
    std::vector<std::vector<DataEntry*>> newTable(newSize);
    
    // iterate through the old table and rehash the entries
    for (int i = 0; i < (int)dataTable.size(); i++) {
        for (int j = 0; j < (int)dataTable[i].size(); j++) {
            DataEntry* entry = dataTable[i][j];
            int newIndex = hash(entry->getCountry(), newSize);
            newTable[newIndex].push_back(entry);
        }
    }

    // clear the old table and update the current size
    dataTable.clear();
    dataTable = std::move(newTable);
    tableSize = newSize;
}
